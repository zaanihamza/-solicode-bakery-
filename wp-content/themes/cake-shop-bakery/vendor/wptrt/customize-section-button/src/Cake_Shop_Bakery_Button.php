...
class Cake_Shop_Bakery_Button extends WP_Customize_Section {
	...
	public function json() {
		...
		if ( ! $this->button_url && $cake_shop_bakery_theme->get( 'ThemeURI' ) ) {

			$button_url = $cake_shop_bakery_theme->get( 'ThemeURI' );

		// Fall back to the `Author URI` defined in `style.css`.
		} elseif ( ! $this->button_url && $cake_shop_bakery_theme->get( 'AuthorURI' ) ) {

			$button_url = $cake_shop_bakery_theme->get( 'AuthorURI' );
		}

		$json['button_text'] = $this->button_text ? $this->button_text : $cake_shop_bakery_theme->get( 'Name' );
